...
// ─────────────────────────────────────────────────────────────────────────────
// Camera intrinsics  (SceauxCastle dataset values)
// ─────────────────────────────────────────────────────────────────────────────
cv::Mat buildCameraMatrix()
{
    return (cv::Mat_<double>(3, 3) <<
...
std::vector<Frame> load_image_sequence(const std::string& dir)
{
    ...
    for (const auto& p : paths) {
        Frame f;
        f.image = cv::imread(p.string(), cv::IMREAD_GRAYSCALE);
        if (f.image.empty()) { std::cerr << "Failed to load: " << p << "\n"; continue; }
        std::cout << "Loaded: " << p.filename() << "\n";
        frames.push_back(f);
    }
    if (frames.size() < 2) std::cerr << "Warning: fewer than 2 images.\n";
...
// ─────────────────────────────────────────────────────────────────────────────
// Cheirality check: returns true if the 3D point lies in front of both cameras.
//   R, t define the camera pose  (X_cam = R * X_world + t)
// ─────────────────────────────────────────────────────────────────────────────
static bool inFront(const cv::Point3f& pt,
                    const cv::Mat& R, const cv::Mat& t)
{
    cv::Mat Xw = (cv::Mat_<double>(3,1) << pt.x, pt.y, pt.z);
    cv::Mat Xc = R * Xw + t;
    return Xc.at<double>(2) > 0.0;
}
                ...
                std::vector<cv::Point3f>& global_pts)
{
    ...
    std::vector<int> idx0, idx1;
    for (const auto& m : matches) {
        pts0.push_back(f0.keypoints[m.queryIdx].pt);
        pts1.push_back(f1.keypoints[m.trainIdx].pt);
        idx0.push_back(m.queryIdx);
        idx1.push_back(m.trainIdx);
    }

    // Show matches (optional – remove if running headless)
    cv::Mat vis;
    cv::drawMatches(f0.image, f0.keypoints, f1.image, f1.keypoints, matches, vis,
                    cv::Scalar::all(-1), cv::Scalar::all(-1), {},
                    cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
    ...
    // Populate landmark maps — with cheirality + finite + depth guard
    // pts3D[k] came from match k  (idx0[k] <-> idx1[k])
    for (int k = 0; k < (int)pts3D.size(); ++k) {
        const auto& p = pts3D[k];
        if (!std::isfinite(p.x) || !std::isfinite(p.y) || !std::isfinite(p.z))
            continue;
        if (std::abs(p.z) > 100.f) continue;
        // FIX #5 – cheirality check for both cameras
        if (!inFront(p, f0.R, f0.t) || !inFront(p, f1.R, f1.t)) continue;
...
// Resection  –  register new_frame using already-registered frames
//
//   Step 1: collect 3D-2D correspondences (deduped by new-frame kp index)
//   Step 2: solvePnPRansac → pose of new_frame
//   Step 3: register inlier landmarks into new_frame.landmark_map
//   Step 4: extend map with new tracks
//           – if one side already has a landmark, propagate it to the other
//           – if neither side is known, triangulate a new point
// ─────────────────────────────────────────────────────────────────────────────
bool resection(Frame&                    new_frame,
               std::vector<Frame*>&      reg_frames,
               const cv::Mat&            K,
               std::vector<cv::Point3f>& global_pts)
{
    ...
    std::vector<cv::Point3f> corr3D;
    std::vector<cv::Point2f> corr2D;
    std::vector<int>         corr_new_kp;
    std::vector<int>         corr_gi;

    // FIX #1 – deduplicate by new-frame keypoint index so each 2D observation
    //          appears only once even when seen by multiple registered frames.
    ...
    for (Frame* ref : reg_frames) {
        ...
        auto matches = keypoint_matching(ref->descriptors,
                                         new_frame.descriptors, MATCHER);
        for (const auto& m : matches) {
            // Skip if this new-frame keypoint already has a correspondence
            if (seen_new_kp.count(m.trainIdx)) continue;
    ...
    // ── Step 3: register inlier landmarks ────────────────────────────────────
    for (int idx : inliers)
        new_frame.landmark_map[corr_new_kp[idx]] = corr_gi[idx];
    ...
    for (Frame* ref : reg_frames) {
        ...
        // Points that need triangulation (both sides unknown)
        std::vector<cv::Point2f> tri_ref_pts, tri_new_pts;
        std::vector<int>         tri_ref_idx, tri_new_idx;

        for (const auto& m : matches) {
            bool ref_known = ref->landmark_map.count(m.queryIdx)      > 0;
            bool new_known = new_frame.landmark_map.count(m.trainIdx) > 0;

            if (ref_known && new_known) {
                // FIX #2a – both known: nothing to do (could merge but skip for now)
                continue;
            }
            else if (ref_known && !new_known) {
                // FIX #2b – ref has the landmark; propagate to new_frame
                new_frame.landmark_map[m.trainIdx] = ref->landmark_map[m.queryIdx];
            }
            else if (!ref_known && new_known) {
                // FIX #2c – new_frame has the landmark; propagate to ref
                ref->landmark_map[m.queryIdx] = new_frame.landmark_map[m.trainIdx];
            }
            else {
                // Both unknown → queue for triangulation
                tri_ref_pts.push_back(ref->keypoints[m.queryIdx].pt);
                tri_new_pts.push_back(new_frame.keypoints[m.trainIdx].pt);
                tri_ref_idx.push_back(m.queryIdx);
                tri_new_idx.push_back(m.trainIdx);
            }
        }

        if ((int)tri_ref_pts.size() < 8) continue;

        // FIX #3 – convert projection matrices to CV_32F for triangulatePoints
        cv::Mat Rt_ref, Rt_new;
        cv::hconcat(ref->R, ref->t, Rt_ref);
        cv::hconcat(new_frame.R, new_frame.t, Rt_new);

        cv::Mat P_ref_d = K * Rt_ref;
        cv::Mat P_new_d = K * Rt_new;
        cv::Mat P_ref_f, P_new_f;
        P_ref_d.convertTo(P_ref_f, CV_32F);
        P_new_d.convertTo(P_new_f, CV_32F);

        cv::Mat pts4D;
        cv::triangulatePoints(P_ref_f, P_new_f, tri_ref_pts, tri_new_pts, pts4D);
        ...
        for (int k = 0; k < pts4D.cols; ++k) {
            ...
            // FIX #4 – cheirality: point must be in front of both cameras
            if (!inFront(p, ref->R, ref->t))            continue;
            if (!inFront(p, new_frame.R, new_frame.t))  continue;

            int gi = (int)global_pts.size();
            global_pts.push_back(p);
            ref->landmark_map[tri_ref_idx[k]]       = gi;
            new_frame.landmark_map[tri_new_idx[k]]  = gi;
        }
    }
...
int main()
{
    ...
    // Two-view initialisation (frames 0 and 1)
    if (!initialise(frames[0], frames[1], K, global_pts)) {
        std::cerr << "Init failed.\n"; return 1;
    }
